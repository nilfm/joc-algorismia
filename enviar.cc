...
struct PLAYER_NAME : public Player {
    ...
    */
    ...
    int distance_bfs(const Pos& start, const Pos& end, bool car) {
        ...
        while (not Q.empty()) {
            ...
            for (int i = 0; i < 8; i++) {
                Dir d = Dir(i);
                Pos p2 = p+d;
                if (p2 == end) return dist;
                if (pos_ok(p2) and not used[p2.i][p2.j] and ((car and car_can_go(p2)) or (not car and warrior_can_go(p2)))) {
    ...
    int distance_dijkstra(const Pos& start, const Pos& end, bool car) {
        VVI used(60, VI(60, false));
        queue<pair<Pos, int>> Q;
        Q.push(make_pair(start, 0));
        used[start.i][start.j] = true;
