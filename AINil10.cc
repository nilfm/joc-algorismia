...
struct PLAYER_NAME : public Player {
    ...
    */
    ...
    int enemy_warriors_around(const Pos& p) {
        ...
        for (int i = -ACCUMULATION_RADIUS; i <= ACCUMULATION_RADIUS; i++) {
			for (int j = -ACCUMULATION_RADIUS; j <= ACCUMULATION_RADIUS; j++) {
				...
				if (pos_ok(curr) and cell_unit(cell(curr)) == 3) count++;
			}
		}
        return count;
    }
    ...
    bool is_better(int curr_dist, int curr_acc, int best_dist, int best_acc) {
		return (curr_dist < best_dist and curr_acc > best_acc); //TO DO
	}
    
   	Dir find_enemy_warrior(const Pos& start) {
		//Primer, trobem la posicio del warrior enemic mes proper
		Pos best(-1, -1);
		int best_acc = 0;
		...
		while (not Q.empty()) {
			Pos curr = Q.front();
			int curr_dist = distances[curr.i][curr.j];
			Q.pop();
			if (cell_unit(cell(curr)) == 3) {
				int acc = enemy_warriors_around(curr);
				if (not pos_ok(best)) {
					best = curr;
					best_acc = acc;
				}
				else if (is_better(curr_dist, acc, distances[best.i][best.j], best_acc)) {
					best = curr;
					best_acc = acc;
				}
			} 
			for (int i = 0; i < 8; i++) {
				Dir d = Dir(i);
				Pos p = curr+d;
				int cost = (cell(curr).type == Road ? 1 : 4);
				int dist = curr_dist+cost;
				if (pos_ok(p) and car_can_go(p) and distances[p.i][p.j] == INF and dist < CAR_RANGE and (distance(start, p) > 2 or is_safe(p, true))) {
					Q.push(p);
					distances[p.i][p.j] = dist;
				}
			}
		}
		//Si no n'hem trobat cap
		if (not pos_ok(best)) return None;
		return dijkstra(start, best, distances);
	}
    
    //Donat un mapa amb distancies, algunes de les quals INF, calcula la direccio de la distancia minima de start a end
    Dir dijkstra(const Pos& start, const Pos& end, const VVI& distances) {
		//Ara tenim a best la posicio del warrior enemic mes proper -> Dijkstra
		priority_queue<IP, vector<IP>, CompPairIP<IP>> PQ;
		VVI distances_from_start(60, VI(60, INF));
        map<Pos, Pos> pare;
        PQ.push(make_pair(0, start));
        pare[start] = start;
        distances_from_start[start.i][start.j] = 0;
        bool cont = true;
        while (not PQ.empty() and cont) {
            Pos p = PQ.top().second;
            int d = PQ.top().first; 
            PQ.pop();
            if (d == distances_from_start[p.i][p.j]) {
                if (p == end) cont = false;
                else {
                    for (int i = 0; i < 8; i++) {
                        Dir di = Dir(i);
                        Pos p2 = p+di;
                        if (pos_ok(p2) and distances[p2.i][p2.j] < INF) {
							int curr_dist = d + distances[p2.i][p2.j];
                            if (curr_dist < distances_from_start[p2.i][p2.j]) {
                                PQ.push(make_pair(curr_dist, p2));
                                distances_from_start[p2.i][p2.j] = curr_dist;
                                pare[p2] = p;
                            }
                        }
                    }
        ...
        for (int c = 0; c < (int)my_cars.size(); c++) {
            ...
            if (can_move(my_cars[c])) {
                if (status(me()) < MAX_STATUS) {
                    ...
                    if (not moved) {
                        Dir d = find_enemy_warrior(curr.pos);
                        if (d != None) {
                            move(my_cars[c], d);
                            moved = true;
                        }
