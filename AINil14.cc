...
struct PLAYER_NAME : public Player {
    ...
    */
    ...
    bool is_safe(const Pos& p, bool car) {
        ...
        if (cell(p).type != City and not car and distance(p, c) < ENEMY_CAR_RANGE) return false;
        ...
        for (int i = 0; i < 60; i++) {
            for (int j = 0; j < 60; j++) {
                ...
                if (not warrior_can_go(here)) w = -1;
                    ...
                    if (dist_city < 8) w += 5*(8-dist_city);
    ...
    bool calculate_aggressive() {
        ...
        if (num_cities_owned < 3 and round() > 50) return true;
        ...
        for (int w = 0; w < (int)my_warriors.size(); w++) {
            ...
            if (status(me()) < MAX_STATUS) {
                ...
                if (not moved and adj_enemy != None) {
                    //cerr << "  Moved to fight enemy" << endl;
